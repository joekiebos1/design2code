...
import { isFigmaUrl } from '../../utils/is-figma-url'
...
export function JioDesignAddClient() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!media || !t || !link) {
      setError('Media, title, and Figma link are required.')
      return
    }
    ...
    if (!isFigmaUrl(link)) {
      setError('Figma link must be a valid https:// URL on figma.com')
      return
    }
    ...
    try {
      ...
      if (uploadFile) {
        ...
        file = await mediaInputToFile(media, 'jio-design')
      }
      ...
      fd.append('inspirationType', 'jioDesign')
      ...
      if (data.id) {
        router.push(`/studio/jio-designs/${encodeURIComponent(data.id)}`)
      }
    } catch (err) {
    ...
    <main className="flex-1 min-h-0 overflow-auto bg-white">
      <div className="max-w-lg mx-auto p-6 md:p-8">
        ...
        <p className="text-sm text-gray-500 m-0 mb-6">
          Add one PNG or MP4 (upload, paste, drop, or direct URL). It is stored in Sanity when you save. Maximum{' '}
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-5">
          ...
          <div>
            <label htmlFor="jd-media-url" className="block text-sm font-medium text-gray-900 mb-1.5">
              Or media URL
            </label>
            <input
              id="jd-media-url"
          ...
          <div>
            <label htmlFor="jd-title" className="block text-sm font-medium text-gray-900 mb-1.5">
              Title *
            </label>
            <input
              id="jd-title"
          ...
          <div>
            <label htmlFor="jd-url" className="block text-sm font-medium text-gray-900 mb-1.5">
              Figma link *
            </label>
            <input
              id="jd-url"
