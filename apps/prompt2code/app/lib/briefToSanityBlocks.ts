 ...
 * spacing, variant, etc.) so Sanity Studio never shows validation errors on new pages.
 */
  ...
  return sections.map((s, i) => {
    ...
    switch (s.component) {
      ...
      case 'mediaText5050': {
        ...
        const imageId = getAsset(imageAssetIds, i)

        let items: Record<string, unknown>[] = []
        if (variant === 'paragraphs' && paragraphColumnLayout === 'multi') {
          items = normalizeItems(slots.items, 'mediaText5050', imageAssetIds, itemOffset)
          itemOffset += items.length
          if (items.length === 0) {
            items = [
              { _type: 'mediaText5050Item', _key: 'mt50-1', subtitle: 'Section 1', body: slots.body ?? '' },
              { _type: 'mediaText5050Item', _key: 'mt50-2', subtitle: 'Section 2', body: '' },
              { _type: 'mediaText5050Item', _key: 'mt50-3', subtitle: 'Section 3', body: '' },
            ]
          }
        }
        ...
        if (variant === 'accordion') {
          accordionItems = (slots.items ?? []).map((item, j) => {
            const o = typeof item === 'object' && item !== null ? (item as Record<string, unknown>) : {}
            return {
              _type: 'mediaText5050AccordionItem',
              _key: `acc-${j}`,
              ...
              image: getAsset(imageAssetIds, itemOffset + j) ? imageRef(getAsset(imageAssetIds, itemOffset + j)!) : undefined,
            }
          })
          if (!accordionItems.length) {
            accordionItems = [
              { _type: 'mediaText5050AccordionItem', _key: 'acc-1', subtitle: 'Panel 1', body: slots.body ?? '' },
              { _type: 'mediaText5050AccordionItem', _key: 'acc-2', subtitle: 'Panel 2', body: '' },
              { _type: 'mediaText5050AccordionItem', _key: 'acc-3', subtitle: 'Panel 3', body: '' },
            ]
          }
      ...
      case 'mediaTextAsymmetric': {
        ...
        const asymImageId = getAsset(imageAssetIds, i)
        let items = normalizeItems(slots.items, 'mediaTextAsymmetric', imageAssetIds, itemOffset)
        itemOffset += items.length
        if (items.length === 0) {
          items = [
            { _type: 'mediaTextAsymmetricItem', _key: 'mta-1', title: 'Question 1', body: 'Answer 1' },
            { _type: 'mediaTextAsymmetricItem', _key: 'mta-2', title: 'Question 2', body: 'Answer 2' },
            { _type: 'mediaTextAsymmetricItem', _key: 'mta-3', title: 'Question 3', body: 'Answer 3' },
          ]
        }
        return {
          ...base,
          ...
          image: asymImageId ? imageRef(asymImageId) : undefined,
          items,
        }
      }
      ...
      case 'cardGrid': {
        ...
        if (items.length === 0) {
          items = [
            { _type: 'cardGridItem', _key: 'cg-1', cardType: 'mediaTextBelow', title: 'Card 1', description: slots.body ?? '', image: getAsset(imageAssetIds, itemOffset) ? imageRef(getAsset(imageAssetIds, itemOffset)!) : undefined },
            { _type: 'cardGridItem', _key: 'cg-2', cardType: 'mediaTextBelow', title: 'Card 2', description: '', image: getAsset(imageAssetIds, itemOffset + 1) ? imageRef(getAsset(imageAssetIds, itemOffset + 1)!) : undefined },
            { _type: 'cardGridItem', _key: 'cg-3', cardType: 'mediaTextBelow', title: 'Card 3', description: '', image: getAsset(imageAssetIds, itemOffset + 2) ? imageRef(getAsset(imageAssetIds, itemOffset + 2)!) : undefined },
          ]
          itemOffset += 3
        }
        return {
