...
const PRODUCT_PAGE_RULES = readFileSync(
  join(process.cwd(), 'lib', 'storytelling', 'product-pages.md'),
  'utf-8'
)
...
Per card: cardType (mediaTextBelow | mediaTextOverlay | colourFeatured | colourIconText | colourTextOnly), title, description, image, video, ctaText, ctaLink.
Slots: title, items[].
...
function buildSystemPromptText(): string {
  const narrativeRoleNote = 'Map: narrativeRole = section (setup | engage | resolve), component = block type, blockOptions = decisions, contentSlots.headline = headline (evocative creative direction).'

  return `You are a senior digital strategist and content architect who specialises in brand websites for companies like Apple, Dyson, and Sonos — brands where storytelling, product beauty, and conversion work together. You help teams turn rough briefs into structured, narrative-led web pages.
...
## Product Page Rules (MUST FOLLOW)
The following rules define how product pages must be structured. Follow them strictly when proposing structure. ${narrativeRoleNote} The output format below overrides any simplified format in the rules — you must output the full PageBrief JSON.

${PRODUCT_PAGE_RULES}

## Product Graph
Use exclusively for CTA destination decisions. Only suggest products that exist here.
...
const SYSTEM_BLOCKS: Anthropic.Messages.TextBlockParam[] = [
  { type: 'text', text: buildSystemPromptText() },
]
...
${conversationSummary ? `Interview conversation:\n${conversationSummary}\n` : ''}
Propose the page structure as JSON. Output ONLY the JSON object, no markdown.`
    ...
    parsed.meta.template = 'product-page'
