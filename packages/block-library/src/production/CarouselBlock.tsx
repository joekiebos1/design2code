...
import { MediaCard, ColourCard, CardLink } from '../components/blocks/Cards'
import type { BlockInteraction } from './CardGridBlock/CardGridBlock.types'

type CarouselItem = {
  cardType?: 'mediaTextBelow' | 'colourFeatured' | null
  title?: string | null
                    ...
                    {displayItems.map((item, i) => {
                      if (isLargeLayout) {
                        ...
                        return (
                          ...
                          >
                            <CardLink interaction={interaction} link={isNav ? item.link : undefined}>
                              {item.cardType === 'colourFeatured' ? (
                                <ColourCard
                                  variant="feature"
                                  title={item.title ?? ''}
                                  description={item.description}
                                  backgroundColor={item.backgroundColor}
                                  cardSize={cardSize === 'large' ? 'large' : cardSize === 'medium' ? 'medium' : 'small'}
                                  aspectRatio={largeAspectRatio === '2:1' ? '4:3' : '4:5'}
                                />
                              ) : (
                                <MediaCard
                                  title={item.title}
                                  ...
                                  video={item.video}
                                  link={isNav ? undefined : item.link}
                          ...
                          <CardLink interaction={interaction} link={isNav ? item.link : undefined}>
                            {item.cardType === 'colourFeatured' ? (
                              <ColourCard
                                ...
                                description={item.description}
                                backgroundColor={item.backgroundColor}
                                cardSize={cardSize === 'medium' ? 'large' : 'small'}
                                aspectRatio={textCardAspectRatio === '8:5' ? '4:3' : '4:5'}
                              />
                            ) : (
                              <MediaCard
                                title={item.title}
