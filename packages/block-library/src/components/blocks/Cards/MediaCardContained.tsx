  ...
  return (
    <SurfaceProvider level={0}>
      ...
      >
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 'var(--ds-spacing-m)', padding: GAP }}>
          {title && <CardTitle>{title}</CardTitle>}
          {description && <CardDescription>{description}</CardDescription>}
          {ctaText && ctaLink && (
            <Button appearance="auto" size="S" attention="low" onPress={() => handleCtaPress(ctaLink)} style={{ alignSelf: 'flex-start', marginTop: 'var(--ds-spacing-m)' }}>
              {ctaText}
            </Button>
          )}
        </div>
