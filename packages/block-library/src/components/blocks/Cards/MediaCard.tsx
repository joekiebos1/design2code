...
export function MediaCard({
  ...
  const imageBlock = (
    <div
      ...
      style={{
        position: 'relative',
        width: '100%',
        ...(heightCss ? { height: heightCss } : { aspectRatio: mediaAspectRatio }),
      }}
    >
      {(hasVideo || hasImage) && (
        <CardMedia
  ...
  const textContent = (
    ...
    >
      {(title || description) && (
        isLarge ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--ds-spacing-m)', width: '100%' }}>
            {title && (
              <p style={{ margin: 0, width: '100%', fontSize: titleFontSize, fontWeight: titleWeight, color: 'var(--ds-color-text-high)', lineHeight: 1.4, whiteSpace: 'pre-line' }}>
                {title}
              </p>
            )}
            {description && (
              <p style={{ margin: 0, width: '100%', fontSize: descFontSize, lineHeight: 1.4, color: 'var(--ds-color-text-low)', fontWeight: 'var(--ds-typography-weight-low)', whiteSpace: 'pre-line' }}>
                {description}
              </p>
            )}
          </div>
        ) : (
          <p style={{ margin: 0, width: '100%', fontSize: titleFontSize, lineHeight: 1.4 }}>
            {title && <span style={{ color: 'var(--ds-color-text-high)', fontWeight: titleWeight, whiteSpace: 'pre-line' }}>{title}</span>}
            {title && description && ' '}
            {description && <span style={{ color: 'var(--ds-color-text-low)', fontWeight: 'var(--ds-typography-weight-low)', whiteSpace: 'pre-line' }}>{description}</span>}
          </p>
        )
      )}
      {ctaText && link && (
        <Button appearance="primary" size="S" attention="low" onPress={() => handleCtaPress(link)} style={{ alignSelf: 'flex-start' }}>
          {ctaText}
        </Button>
      )}
    </div>
  ...
  const textBlock = (title || description || (ctaText && link))
    ? isLarge
      ? (
          <WidthCap contentWidth="S" style={{ flex: 1, minWidth: 0, minHeight: 0, width: '100%', marginInline: 0, textAlign: 'left', alignSelf: 'flex-start' }}>
            {textContent}
          </WidthCap>
        )
      : textContent
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: layout === 'compact' ? 'var(--ds-spacing-m)' : GAP, width: '100%', height: '100%', minHeight: 0 }}>
      {link && link.trim() !== '' && !ctaText ? (
        <Link href={link.trim()} style={{ textDecoration: 'none', color: 'inherit', display: 'block', flexShrink: 0 }}>
          {imageBlock}
        </Link>
      ) : (
