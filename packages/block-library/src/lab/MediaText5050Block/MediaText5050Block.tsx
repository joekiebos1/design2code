...
import { WidthCap, type ContentWidth } from '../../production/WidthCap'
...
import { EDGE_TO_EDGE_BREAKOUT } from '@design2code/ds'
...
import type {
  ...
  MediaText5050Item,
  MediaText5050Media,
} from '../../production/MediaText5050Block/MediaText5050Block.types'
import {
...
import accordionStyles from '../../production/MediaText5050Block/MediaText5050Accordion.module.css'
...
export function MediaText5050Block({
  ...
  const variant = rawVariant === 'singleParagraph'
    ? 'paragraphs' as const
    : rawVariant === 'multiParagraph'
      ? 'paragraphs' as const
      : rawVariant === 'paragraphs' || rawVariant === 'accordion'
        ? rawVariant
        : 'paragraphs' as const
  const useSingleLayout = rawVariant === 'singleParagraph' || (rawVariant === 'paragraphs' && paragraphColumnLayout === 'single')
  ...
  const framingTextAlign = resolvedHeadingAlignment === 'center' ? 'center' : 'left'
  ...
  const textColumnStyle: CSSProperties = {
    ...
    flexDirection: 'column',
    gap: 'var(--ds-spacing-m)',
    alignItems: resolvedHeadingAlignment === 'center' ? 'center' : 'flex-start',
    minWidth: 0,
    ...(!isStacked &&
      (mediaFirst ? { paddingLeft: TEXT_COLUMN_INSET } : { paddingRight: TEXT_COLUMN_INSET })),
  }

  const bgColor = useBlockBackgroundColor(emphasis, appearance)
  ...
  const paragraphsContent = useSingleLayout ? (
    ...
    <Grid as="section">
      <div style={{ ...textOnlyCell, display: 'flex', flexDirection: 'column', alignItems: 'stretch' }}>
        {blockFramingIntro != null ? (
          <div style={{ marginBottom: 'var(--ds-spacing-4xl)', width: '100%' }}>{blockFramingIntro}</div>
        ) : null}
        {textContent}
      </div>
    </Grid>
  ) : isStacked ? (
    <section style={{ display: 'flex', flexDirection: 'column', gap: 'var(--ds-spacing-4xl)' }}>
      {blockFramingIntro != null ? (
        <WidthCap
          contentWidth="L"
          style={resolvedHeadingAlignment === 'left' ? { marginInline: 0 } : undefined}
        >
