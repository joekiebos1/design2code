...
function getLabAsymmetricSectionVariant(
  ...
  const sectionIndex = path[1]
  if (typeof sectionIndex !== 'number') return undefined
  return sections[sectionIndex]?.variant
}

export const labMediaTextAsymmetricItem = defineType({
  name: 'labMediaTextAsymmetricItem',
  type: 'object',
  title: 'Item',
  fields: [
    defineField({
      ...
      type: 'string',
      title: 'Title',
      description: 'FAQ: question. Links: hidden.',
      hidden: ({ document, path }) =>
        getLabAsymmetricSectionVariant(document as { sections?: Array<{ variant?: string }> }, path as unknown[]) === 'links',
    }),
    defineField({
      name: 'body',
      type: 'text',
    ...
    defineField({
      ...
      hidden: ({ document, path }) =>
        getLabAsymmetricSectionVariant(document as { sections?: Array<{ variant?: string }> }, path as unknown[]) === 'faq',
    }),
    defineField({
      name: 'subtitle',
      type: 'string',
...
export const labMediaTextAsymmetricBlock = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: 'Items',
      group: 'content',
      of: [{ type: 'labMediaTextAsymmetricItem' }],
