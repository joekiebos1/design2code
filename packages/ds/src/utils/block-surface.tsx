 ...
 * - **appearance**: primary | secondary | sparkle | neutral — colour theme (aligned with Figma)
 * - **theme**: design system theme (optional, per block)
 *
 * ## Renderer mapping
 * BlockRenderer and LabBlockRenderer pass block.emphasis and block.appearance directly to block props.
 * Queries use coalesce(appearance, surfaceColour) until legacy documents are migrated.
 *
 * ## Block props
 * All blocks receive: emphasis, appearance (and theme where applicable).
 *
...
/** Block appearance: primary, secondary, sparkle, neutral. Maps to DS appearance tokens. Aligned with Figma + CMS field `appearance`. */
export type BlockAppearance = 'primary' | 'secondary' | 'sparkle' | 'neutral'

/** @deprecated Use BlockAppearance */
export type SurfaceColour = BlockAppearance

/** @deprecated Use Emphasis instead. */
export type BlockSurface = Emphasis

/** @deprecated Use BlockAppearance instead. */
export type BlockAccent = BlockAppearance
...
function resolveBlockBackgroundColor(
  ...
  const appearanceMap: Record<BlockAppearance, 'Primary' | 'Secondary' | 'Sparkle' | 'Neutral'> = {
    primary: 'Primary',
    secondary: 'Secondary',
    sparkle: 'Sparkle',
    neutral: 'Neutral',
...
export type BlockSurfaceProviderProps = {
  ...
  surfaceColour?: BlockAppearance | null
...
export function BlockSurfaceProvider({
  ...
  const resolvedAppearance = appearance ?? surfaceColour
  const surfaceProps = getSurfaceProviderProps(emphasis)
  const bgColor = useBlockBackgroundColor(emphasis, resolvedAppearance)
  ...
  if (bgColor) {
    ...
    if (fullWidth) {
      return (
        ...
        >
          <div style={{ ...edgeStyles.inner, overflowX: edgeStyles.isCapped ? 'hidden' : 'visible' }}>{content}</div>
        </div>
      )
    }
